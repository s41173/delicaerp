...
//    fungsi di panggil ketika ada po yg masih blm approved ketika hendak closing harian
    ...
    {
        $this->ci->db->select('id, request, product, qty, desc, request_date, user, status');
        $this->ci->db->from('request_item');
        $this->ci->db->where('request', $no);
        $this->ci->db->where('status', 'accept');
