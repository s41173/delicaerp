...
//    fungsi di panggil ketika ada po yg masih blm approved ketika hendak closing harian
    ...
    function get_so($no)
    {
        $this->ci->db->select('no, p1, p2, costs, status, total, tax, discount, notes, docno, currency, customer');
    ...
    function get_contract_no($no)
    {
       $this->ci->db->select('no,notes,dates,total,contract_no');
       $this->ci->db->where('no', $no);
       $query = $this->ci->db->get('sales')->row();
       return $query->contract_no;
    }
