...
class Ar_installment extends MX_Controller
{
	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last_ar_installment');
            ...
            $this->table->set_heading('No', 'Code', 'Date', 'Sales - No', 'AR - Payment', 'Currency', 'Amount');
            ...
            foreach ($ar_installments as $ar_installment)
            {
                ...
                $this->table->add_row
                (
                    ++$i, 'IAS-00'.$ar_installment->no, tgleng($ar_installment->dates), 'SO-00'.$ar_installment->sales_no, 'CR-00'.$ar_installment->ar_payment, $this->get_so_currency($ar_installment->sales_no), number_format($ar_installment->amount)
                );
            }
