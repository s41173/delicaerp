...
<title> AR-Payment - CR-00<?php echo isset($pono) ? $pono : ''; ?></title>
<style media="all">
			...
			if ($items)
			{
				...
				foreach ($items as $res)
				{
					echo "
					 ...
					 <tr> 
						<td> ".$i." </td>
						<td class=\"left\"> CO-00".$sales->get_contract_no($res->no).' / '.$res->code."-00".$res->no." - ".$res->notes." &nbsp; - &nbsp; ".tgleng($res->dates)." <br> <p class=\"notes\">". $res->transnotes ."</p> </td> 
						<td class=\"right\"> ".number_format($res->amount-$res->tax+$res->tax2+$res->cost)." </td>
						<td class=\"right\"> ".number_format($res->tax)." </td>
						<td class=\"center\"> <br> Cost : <br> Deduction : </td> 
						<td class=\"right\"> ".number_format($res->amount+$res->cost+$res->tax2)." <br> (".number_format($res->cost).") <br> (".number_format($res->tax2).") </td>  
					 </tr>
