...
class Payment_trans_model extends CI_Model
{
    ...
    {
        $this->db->select('id, ar_payment, code, cash, no, sid, notes, cost, tax, tax2, discount, amount');
    ...
    function get_po_details($po)
    {
        $this->db->select('ar_payment_trans.id, ar_payment_trans.ar_payment, ar_payment_trans.code, ar_payment_trans.cash, ar_payment_trans.no,
                          ar_payment_trans.sid, ar_payment_trans.notes as transnotes,  ar_payment_trans.cost, ar_payment_trans.tax2, ar_payment_trans.discount, ar_payment_trans.tax, ar_payment_trans.amount,
    ...
    function report($customer,$start,$end,$acc,$cur)
    {
        $this->db->select('ar_payment.id, ar_payment.no, ar_payment.docno, ar_payment.check_no, ar_payment.dates, customer.prefix, customer.name, ar_payment.user,
                           ...
                           ar_payment_trans.code, ar_payment_trans.sid, ar_payment_trans.notes as transnotes, ar_payment_trans.no as transno, ar_payment_trans.cost, ar_payment_trans.tax2, ar_payment_trans.discount, ar_payment_trans.tax, ar_payment_trans.amount');
    ...
    {
        $this->db->select('id, ar_payment, code, cash, no, sid, cost, tax, tax2, discount, amount');
        $this->db->from($this->table);
        $this->db->where('no', $no);
