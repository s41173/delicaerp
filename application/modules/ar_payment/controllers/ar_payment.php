	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last');
            ...
            foreach ($appayments as $appayment)
            {
                ...
                $this->table->add_row
                (
                    ++$i, 'CR-00'.$appayment->no, tglin($appayment->dates), $appayment->prefix.' '.$appayment->name, $this->acc($appayment->acc).' - '.$appayment->currency, $appayment->check_no, number_format($appayment->amount),
...
//
            ...
            else
            {
               ...
               $cost       = $cm->get_id(53);
               $piutangtax = $cm->get_id(54);
               $account    = $appayment->account;
                
               $this->journalgl->new_journal('00000'.$appayment->no,$appayment->dates,'SO',$appayment->currency, 'Customer payment for : '.$this->get_trans_code($appayment->no).' - '.$appayment->prefix.' '.$appayment->name, $appayment->amount, $this->session->userdata('log'));
               $dpid = $this->journalgl->get_journal_id('SO','00000'.$appayment->no);
                
               $this->journalgl->add_trans($dpid,$ar,0,intval($appayment->amount+$sum['discount']+$sum['cost']+$sum['tax2'])); // kredit piutang
               $this->journalgl->add_trans($dpid,$account,$appayment->amount,0); // bank masuk
               if ($sum['discount'] > 0){ $this->journalgl->add_trans($dpid,$discount,$sum['discount'],0); }
               ...
               $this->session->set_flashdata('message', "$this->title SO-00000$appayment->no confirmed..!"); // set flash data message dengan session
               redirect($this->title);
            }
        }
    ...
    private function settled_so($no,$dates=null)
    {
        ...
        $sum = $this->Payment_trans_model->total($no);
        $data = array('status' => 1, 'p2' => 0);

        foreach ($vals as $val)
        {
           if ($val->cash == 1) { $this->sales->settled_so($val->sid,$data); }
           elseif ($val->cash == 0) { $this->settled_credit_so($val->sid,intval($val->amount+$sum['cost']+$sum['tax2']),$dates,$no); }
    ...
    private function settled_credit_so($sid,$amount,$dates,$ar_payment)
    {
        ...
        if ($p2 <= 0) { $result = array('p1' => $p1, 'p2' => $p2, 'status' => 1); }
        else { $result = array('p1' => $p1, 'p2' => $p2); }
    ...
    private function unsettled_so($no,$dates=null)
    {
        ...
        $sum = intval($total['amount']+$total['cost']+$total['tax2']);
        $data = array('status' => 0, 'p2' => $sum);

        foreach ($vals as $val)
        { 
            if ($val->cash == 1) { $this->sales->settled_so($val->sid,$data); }
            elseif ($val->cash == 0) { $this->unsettled_credit_so($val->sid,$sum,$dates,$no); }
    ...
    private function unsettled_credit_so($sid,$amount,$dates,$ar_payment)
    {
        ...
        if ($sales->status == 1) { $result = array('status' => 0); }
        else { $result = array('p1' => $p1, 'p2' => $p2, 'status' => 0); }
...
//    ===================== approval ===========================================
    ...
    private function rollback($uid,$po)
    {
       ...
       $this->journalgl->remove_journal('SO', '00000'.$po); // delete journal gl
...
//            echo validation_errors();
	...
	$data['form_action'] = site_url($this->title.'/update_process/'.$po);
        ...
        if ($appayment->acc == 'bank'){ $data['bank'] = $this->account->combo_based_classi(8); }else { $data['bank'] = $this->account->combo_based_classi(7); }
...
//        ============================ Item  =========================================
        ...
        //Set heading untuk table
        $this->table->set_heading('No', 'Code', 'Type', 'Cost', 'Tax', 'Discount', 'Other Tax', 'Amount', 'Notes', 'Action');
...
//        $this->db->select('id, ap_payment, code, no, notes, amount');
        ...
        foreach ($items as $item)
        {
            $this->table->add_row
            (
                ++$i, $item->code.'-00'.$item->no, $this->get_cash_status($item->cash), number_format($item->cost), number_format($item->tax), number_format($item->discount), number_format($item->tax2), number_format($item->amount), $item->notes,
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            ...
            if ($this->input->post('ctype') == 1)
            {
                $pitem = array('ar_payment' => $po, 'code' => 'SO', 'no' => $sales->no, 'sid' => $this->input->post('titem'), 'cash' => $this->input->post('ctype'),
                               'tax' => $sales->tax, 'discount' => $sales->discount, 'cost' => $sales->costs, 'notes' => $this->input->post('tnotes'),
            ...
            else
            {
                ...
                $pitem = array('ar_payment' => $po, 'code' => 'SO', 'no' => $sales->no, 'sid' => $this->input->post('titem'), 'cash' => $type, 'notes' => $this->input->post('tnotes'),
                               'amount' => intval($this->input->post('tamount')+$this->input->post('ttax')-$this->input->post('tcost')-$this->input->post('ttax2')-$this->input->post('tdiscount')), 
                               'discount' => $this->input->post('tdiscount'), 'tax' => $this->input->post('ttax'), 'cost' => $this->input->post('tcost'),'tax2' => $this->input->post('ttax2'));
            }
