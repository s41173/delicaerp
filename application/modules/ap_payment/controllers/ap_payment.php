...
class Ap_payment extends MX_Controller
{
    ...
    private $vendor,$user,$journal,$cek,$purchase,$purchase_return, $currency, $account;
	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last');
            ...
            foreach ($appayments as $appayment)
            {
                ...
                $this->table->add_row
                (
                    ++$i, 'CD-0'.$appayment->no, $this->get_type($appayment->tax), tglin($appayment->dates), $appayment->prefix.' '.$appayment->name, $this->acc($appayment->acc).' - '.$appayment->currency, $appayment->check_no, number_format($appayment->amount+$appayment->over,2),
...
//
            ...
            elseif ($this->cek_po_settled($appayment->no) == FALSE || $this->cek_pr_settled($appayment->no) == FALSE)
            {
...
//                $this->settled_pr($appayment->no); // fungsi untuk mensettled kan semua pr
    ...
    {
        $vals = $this->Payment_trans_model->get_last_trans($no,'PO')->result();
    ...
    private function cek_po_settled($no)
    {
        ...
        foreach ($vals as $val)
        {
            if ($this->purchase->cek_settled($val->no) == FALSE)
            {
                $res = FALSE;
                break;
            }
...
//    ===================== approval ===========================================
    ...
    function delete($uid,$po)
    {
        ...
        {
            if ($appayment->approved == 1){ $this->rollback($uid, $po); $this->session->set_flashdata('message', "1 $this->title successfully rollback..!");  }
...
//        $this->form_validation->set_rules('tcheck', 'Check No', 'callback_valid_check|callback_valid_check_no');
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            $appayment = array('vendor' => $this->vendor->get_vendor_id($this->input->post('tvendor')),
                               'no' => $this->Appayment_model->counter_no(), 'tax' => $this->input->post('rtype'),
...
//        $this->db->select('id, ap_payment, code, no, notes, amount');
        ...
        foreach ($items as $item)
        {
            $this->table->add_row
            (
                ++$i, $item->code.'-00'.$item->no, number_format($item->discount,2), number_format($item->amount,2),
...
//    ======================  Item Transaction   ===============================================================
    ...
    function add_item($po=null)
    {
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            ...
            $pitem = array('ap_payment' => $po, 'code' => 'PO', 'no' => $this->input->post('titem'), 'discount' => $this->input->post('tdiscount'), 'amount' => $this->calculate_rate($po,$amount));
    ...
    private function update_trans($po)
    {
        ...
        $totalpr = $this->Payment_trans_model->total_pr($po);
	...
	// Form validation
        ...
        {   
            $appayment = array('log' => $this->session->userdata('log'), 'acc' => $this->input->post('cacc'), 'dates' => $this->input->post('tdate'), 'account' => $this->input->post('cbank'), 'late' => $this->input->post('tlate'),
                               'due' => $this->input->post('tdue'), 'post_dated' => $this->input->post('cpost'), 'check_no' => $this->cek_null($this->input->post('tcheck')));
...
//            $this->load->view('appayment_transform', $data);
    ...
    {
        if ($this->Payment_trans_model->get_item_based_po($no,'PO',$po) == FALSE)
        {
...
//
       ...
       $data['amount'] = number_format($appayment->amount,2);
       $data['late'] = number_format($appayment->late,2);
       $data['over'] = number_format($appayment->over,2);
       $data['check'] = $appayment->check_no;
       $data['due'] = tglin($appayment->due);
//
       $data['items'] = $this->Payment_trans_model->get_po_details($po)->result();

       $terbilang = $this->load->library('terbilang');
       
       $amount = explode('.',$appayment->amount+$appayment->over);
       if ($appayment->currency == 'IDR')
       { $data['terbilang'] = ucwords($terbilang->baca($amount[0])).' Rupiah'; }
       else { $data['terbilang'] = ucwords($terbilang->baca($amount[0])); }
