...
//        ============================ Purchase Item  =========================================
        ...
        //Set heading untuk table
        $this->table->set_heading('No', 'Product', 'Qty', 'Desc', 'Date', 'User', 'Vendor', 'Action');
        ...
        foreach ($items as $item)
        {
            $this->table->add_row
            (
                ++$i, $this->product->get_name($item->product), $item->qty.' '.$this->product->get_unit($item->product), $item->desc, tglin($item->demand_date), $item->user, $this->vendor->get_vendor_name($item->vendor),
...
//        Property Details
        ...
        if ($type == 0){ $data['reports'] = $this->Demand_model->report($start,$end)->result(); $this->load->view('demand_report_details', $data);  } 
        else { $data['reports'] = $this->Demand_item_model->report_category($start,$end)->result(); $this->load->view('demand_report_category', $data); }
