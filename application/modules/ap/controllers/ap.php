...
class Ap extends MX_Controller
{
    ...
    private $properti, $modul, $title, $cost, $bank,$ps, $model, $ledger, $account;
...
//                $datax = array('name'=> 'cek[]','id'=> 'cek'.$i,'value'=> $ap->id,'checked'=> FALSE, 'style'=> 'margin:0px');
                ...
                $this->table->add_row
                (
                    ++$i, 'DJ-00'.$ap->no, $ap->currency, tglin($ap->dates), $this->vendor->get_vendor_name($ap->vendor), $ap->notes, ucfirst($ap->acc), number_format($ap->amount), $this->status($ap->status),
...
//    ===================== approval ===========================================
	...
	// Form validation
        ...
        {
            $trans = array('vendor' => $this->vendor->get_vendor_id($this->input->post('tvendor')), 
...
//            echo validation_errors();
	...
	$data['form_action'] = site_url($this->title.'/update_process/'.$ap->id);
        ...
        if ($ap->acc == 'bank'){ $data['bank'] = $this->account->combo_based_classi(8); }
...
//        $this->cek_confirmation($po,'add_trans');
	...
	// Form validation
        ...
        { 
            ...
            $this->model->due      = $this->input->post('tdue');
