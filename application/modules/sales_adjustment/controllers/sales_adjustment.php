...
class Sales_adjustment extends MX_Controller
{
	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last_sales_adjustment');
            ...
            $this->table->set_heading('No', 'Code', 'Date', 'Sales', 'Currency', 'Notes', 'DP', 'Total', 'Balance', 'Log', 'Action');
            ...
            foreach ($sales_adjustments as $sales_adjustment)
            {
                ...
                $this->table->add_row
                (
                    ++$i, 'SAJ-00'.$sales_adjustment->no, tgleng($sales_adjustment->dates), 'SO-00'.$sales_adjustment->sales_no, $sales_adjustment->currency, $sales_adjustment->notes, number_format($sales_adjustment->dp), number_format($sales_adjustment->total), number_format($sales_adjustment->amount), $sales_adjustment->log,
...
//    ===================== approval ===========================================
    ...
    function confirmation($pid)
    {
        ...
        else
        {
          // $this->sales->delete_so_item($sales_adjustment->sales_no); // delete so item
           ...
           $this->sales->update_id($sales_adjustment->sales_no,$sales); // update so

           //  create journal
           $this->create_so_journal($sales_adjustment->dates, $sales_adjustment->currency, 'SAJ-00'.$sales_adjustment->sales_no.'-'.$sales_adjustment->notes, 'SAJ',
                                    $sales_adjustment->no, 'AR', $sales_adjustment->total, $sales_adjustment->dp, $sales_adjustment->amount);
    ...
    private function create_so_journal($date,$currency,$code,$codetrans,$no,$type,$total,$p1,$amount)
    {
       ...
       $this->journalgl->new_journal($no,$date,'SAJ',$currency,$code,$amount, $this->session->userdata('log'));
       $jid = $this->journalgl->get_journal_id('SAJ',$no);
       
       $this->journalgl->add_trans($jid,$cost,$amount,0); // tambah beban
       $this->journalgl->add_trans($jid,$ar,0,$amount); // kurang piutang
...
//    ===================== approval ===========================================
    ...
    function delete($uid,$po)
    {
        ...
        {
            $this->journalgl->remove_journal('SAJ',$po); // delete journal
            $sales = array('status' => 0);
            $this->sales->update_id($val->sales_no,$sales); // rollback sales status
            
            $data = array('approved' => 0);
            $this->Sales_adjustment_model->update_id($uid, $data); // rollback status
        }
        else
...
//            echo validation_errors();
	...
	$data['form_action'] = site_url($this->title.'/update_process/'.$po);
        ...
        $data['default']['amount'] = $sales_adjustment->amount;
	...
	// Form validation
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
             $sales_adjustment = array('docno' => $this->input->post('tdocno'), 'sales_no' => $this->input->post('tsales'),
                                       ...
                                       'notes' => $this->input->post('tnote'), 'desc' => $this->input->post('tdesc'), 'amount' => $this->input->post('tamount'),
                                       'user' => $this->user->get_userid($this->input->post('tuser')), 'log' => $this->session->userdata('log'));
