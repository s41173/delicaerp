...
//        $this->journal = new Journal_lib();
        ...
        $this->request = new Request_lib();

    }

    private $properti, $modul, $title, $request;
	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last_purchase');
            ...
            foreach ($purchases as $purchase)
            {
                ...
                $this->table->add_row
                (
                    ++$i, 'PO-00'.$purchase->no, $purchase->currency, tglin($purchase->dates), ucfirst($purchase->acc), $purchase->prefix.' '.$purchase->name, $purchase->notes, number_format($purchase->total + $purchase->costs,2), number_format($purchase->p2,2), $this->status($purchase->status),
...
//    ===================== approval ===========================================
	...
	// Form validation
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            ...
            $purchase = array('vendor' => $this->vendor->get_vendor_id($this->input->post('tvendor')), 'no' => $this->input->post('tno'), 'request' => $this->input->post('tpr'),
...
//        ============================ Purchase Item  =========================================
        ...
        foreach ($items as $item)
        {
            $this->table->add_row
            (
                ++$i, 'PRO-0'.$item->product.' : '.$this->product->get_name($item->product), $item->qty, $this->product->get_unit($item->product), number_format($item->price,2), number_format($item->tax), number_format($item->amount,2),
