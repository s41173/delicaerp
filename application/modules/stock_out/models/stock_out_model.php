...
//    =========================================  REPORT  =================================================================
    ...
    {
        $this->db->select('stock_out.id, stock_out.no, stock_out.dates, stock_out.currency, stock_out.desc, stock_out.user, stock_out.balance, stock_out.approved, stock_out.log,
                           stock_out_item.product, stock_out_item.qty, stock_out_item.price, stock_out_item.desc, stock_out_item.staff');
    ...
    function report_project($start,$end,$project)
    {
        $this->db->select('stock_out.id, stock_out.no, stock_out.dates, stock_out.currency, stock_out.desc, stock_out.user, stock_out.balance, stock_out.approved, stock_out.log,
                           stock_out_item.product, stock_out_item.qty, stock_out_item.price, stock_out_item.desc, stock_out_item.staff');
        ...
        $this->db->where("stock_out.dates BETWEEN '".setnull($start)."' AND '".setnull($end)."'");
        $this->db->where('stock_out.approved', 1);
        $this->db->where('stock_out_item.desc', $project);
        $this->db->order_by('stock_out.no', 'asc');
        return $this->db->get();
    }
