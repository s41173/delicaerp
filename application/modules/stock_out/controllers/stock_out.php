...
class Stock_out extends MX_Controller
{
    ...
    private $properti, $modul, $title, $currency, $stockvalue=0,$reststock, $project, $log;
...
//            echo validation_errors();
	...
	$data['form_action'] = site_url($this->title.'/update_process/'.$po);
        ...
        $data['code'] = $po;
        $data['project'] = $this->project->combo();
...
//        ============================ Purchase Item  =========================================
        ...
        //Set heading untuk table
        $this->table->set_heading('No', 'Product', 'Qty', 'Desc', 'Amount', 'Staff', 'Action');
        ...
        foreach ($items as $item)
        {
            $this->table->add_row
            (
                ++$i, $this->product->get_name($item->product), $item->qty.' '.$this->product->get_unit($item->product), $item->desc, number_format($item->price), $item->staff,
...
//            echo $this->stockvalue;
            ...
            $pitem = array('product' => $product, 'stock_out' => $po, 'price' => intval($this->stockvalue/$this->input->post('tqty')),
                           'qty' => $this->input->post('tqty'), 'desc' => $this->input->post('cproject'), 'staff' => $this->input->post('tstaff'));
    ...
    function add_barcode($po=null)
    {
        $this->cek_confirmation($po,'add_trans');
        
        $data['form_action_item'] = site_url($this->title.'/add_barcode_process/'.$po);
        $data['code'] = $po;
        $data['project'] = $this->project->combo();
        $this->load->view('stockout_barcode_form', $data);
    }
    
    function add_barcode_process($po=null)
    {
        $this->cek_confirmation($po,'add_trans');
        
        $this->form_validation->set_rules('tproduct', 'Item Name', 'required');
        $this->form_validation->set_rules('tqty', 'Qty', 'required|numeric|callback_valid_qty_barcode');
        $this->form_validation->set_rules('tdesc', 'Desc', '');

        if ($this->form_validation->run($this) == TRUE)
        {
           $res = explode('PRO-0', $this->input->post('tproduct'));
           
           if (@$res[1])
           {
             $val = $this->product->valid_id($res[1]);
             if ($val == TRUE)
             { 
                $this->get_stock($val, $this->input->post('tqty'), $po);
                $pitem = array('product' => $res[1], 'stock_out' => $po, 'price' => intval($this->stockvalue/$this->input->post('tqty')),
                               'qty' => $this->input->post('tqty'), 'desc' => $this->input->post('cproject'), 'staff' => $this->input->post('tstaff'));
...
//        $this->cek_confirmation($po,'add_trans');
	...
	// Form validation
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            ...
            // insert log
            $this->log->insert($this->session->userdata('userid'), $this->input->post('tdate'), waktuindo(), 'update', 'BPBG-00'.$this->input->post('tno'));
...
//            $this->load->view('stockout_transform', $data);
    ...
    public function valid_qty_barcode($qty)
    {
        if ($this->input->post('tproduct'))
        {
           $res = explode('PRO-0', $this->input->post('tproduct')); 
           $pro = $this->product->get_details($res[1]);
           $pqty = $pro->qty; 
           if ($pqty - $qty < 0) { $this->form_validation->set_message('valid_qty', "Qty Not Enough..!"); return FALSE; } else { return TRUE; }
        }
        else
        {
           $this->form_validation->set_message('valid_qty', "Product Required..!");
           return FALSE;
        }
    }
...
// ====================================== REPORT =========================================
    ...
    function report_process()
    {
        ...
        $project = $this->input->post('cproject');
        ...
        if ($project){
            $pro = $this->project->get_details($project);
            $data['prono']   = 'PRJ-00'.$pro->no;
            $data['procust'] = $cust->get_customer_name($pro->customer);
            $data['prodate'] = tglin($pro->dates);
            $data['prolocation'] = $pro->location;
            $data['prodesc']     = $pro->desc;
            $data['prostatus'] = $pro->status;
            $data['prostaff'] = $pro->staff;
            $data['prolog'] = $pro->log;
        }
...
//        Property Details
        ...
        $data['items'] = $this->Stock_out_model->report_project($start,$end,$project)->result();
        ...
        elseif ($type == 4){ $this->load->view('stockout_project_invoice', $data);}
