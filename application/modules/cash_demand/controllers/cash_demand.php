...
//                $datax = array('name'=> 'cek[]','id'=> 'cek'.$i,'value'=> $ap->id,'checked'=> FALSE, 'style'=> 'margin:0px');
                ...
                $this->table->add_row
                (
                    ++$i, 'RC-00'.$ap->no, $ap->currency, tglin($ap->dates), $ap->notes, ucfirst($ap->acc), number_format($ap->amount),
...
//    ===================== approval ===========================================
    ...
    function delete($uid,$po)
    {
        ...
        if ($val->approved == 1){ $this->session->set_flashdata('message', "1 $this->title successfully approved..!"); redirect($this->title);  }
