  ...
  function types()
	{
		...
		if (res == "stock"){ document.getElementById("tproduct").readOnly = true; }
		else{ document.getElementById("tproduct").readOnly = false; }
	}
...
<body onload="types();" onUnload="refreshparent();">
<div id="webadmin">
	...
	<fieldset class="field" style="float:left;">  <legend> Purchase Request </legend>
	<form name="modul_form" class="myform" id="ajaxform" method="post" action="<?php echo $form_action; ?>">
	     ...
	     <td> <input type="text" class="required" readonly name="tno" size="4" title="No" value="<?php echo isset($code) ? $code : ''; ?>" /> &nbsp; <br /> </td>
					...
					<tr>
						 <td> <label for="tdate"> Type </label> </td> <td>:</td>
						 <td>
							 <select name="ctype" id="ctype" onchange="types();">
								 <option value="stock" <?php echo set_select('ctype', 'stock', isset($default['type']) && $default['type'] == 'stock' ? TRUE : FALSE); ?> /> Stock </option>
								 <option value="other" <?php echo set_select('ctype', 'other', isset($default['type']) && $default['type'] == 'other' ? TRUE : FALSE); ?> /> Other </option>
							 </select> &nbsp; <br />
						</td>
					</tr>
	...
	<form name="modul_form" class="myform" id="ajaxform2" method="post" action="<?php echo $form_action_item; ?>">
		<table>
			<tr>
				...
				<td>
					<label for="tdesc"> Desc / Project : </label> <br />
					<input type="text" name="tdesc" size="30" title="Desc" /> &nbsp;
				</td>

				<td> <label for="tdate"> Demand Date </label> <br>
				   <input type="Text" name="trequestdate" id="d2" title="Demand date" size="10" class="required"
                ...
                <td>
					<label for="tuser"> User : </label> <br />
					<input type="text" name="tuser" size="15" title="User" /> &nbsp;
				</td>
