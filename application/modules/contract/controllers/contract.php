...
//    ===================== approval ===========================================
    ...
    {
//      $this->Phase_model->delete_po($po); 
      $contract = $this->Contract_model->get_contract_by_no($po)->row();
      $sales = $this->sales->valid_sales_contract($po);
      ...
      if ($sales == TRUE)
      {
        ...
        else { $this->session->set_flashdata('message', "1 $this->title payment hasn't finished, can't removed..!");  }  
      }
      else
      {
         $this->Contract_model->delete($uid);  
...
//    ======================  Item Transaction   ===============================================================
    ...
    function add_item($po=null)
    {
        ...
        $this->form_validation->set_rules('tpart', 'Part', 'required|numeric');
        $this->form_validation->set_rules('tpartdate', 'Part Date', 'required');
        $this->form_validation->set_rules('tpartamount', 'Amount', 'required|numeric');

        if ($this->form_validation->run($this) == TRUE)
        {
            $pitem = array('no' => $this->input->post('tpart'), 'contract' => $po, 'dates' => $this->input->post('tpartdate'),
                           'amount' => $this->input->post('tpartamount'), 'status' => 0);
            $this->Phase_model->add($pitem);
    ...
    {
        $totals = $this->Phase_model->total($po);
        $contract = array('amount' => $totals['amount']);
    ...
    function delete_item($id,$po)
    {
        ...
        $this->Phase_model->delete($id); // memanggil model untuk mendelete data
        $this->update_trans($po);
