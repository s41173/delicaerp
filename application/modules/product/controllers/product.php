...
//
    ...
    private $purchase, $brand, $category, $unit, $vendor, $purchase_item, $stock_out_item, $opname, $wt;
     ...
     private $atts = array('width'=> '250','height'=> '200',
                      'scrollbars' => 'no','status'=> 'no',
                      'resizable'=> 'no','screenx'=> '0','screenx' => '\'+((parseInt(screen.width) - 250)/2)+\'',
                      'screeny'=> '0','class'=> 'barcode','title'=> '', 'screeny' => '\'+((parseInt(screen.height) - 200)/2)+\'');
...
//      // tangkap variabel keyword dari URL
	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last_product');
            ...
            $this->table->set_heading('#','No', 'Code', 'Storage', 'Brand', 'Category', 'Cur', 'Type', 'Model', 'Name', 'Qty', 'Unit', 'Price', 'Total', 'Action');
            ...
            foreach ($products as $product)
            {
                ...
                $this->table->add_row
                (
                    form_checkbox($datax), ++$i, 'PRO-0'.$product->id, 'WH-0'.$product->warehouse_id, $this->brand->get_name($product->brand), $this->category->get_name($product->category), $product->currency, ucfirst($product->type), $product->model, $product->name, $product->qty.' '.$product->unit, number_format($this->get_unit_cost($product->id)), number_format($product->price), number_format($this->get_unit_cost($product->id)*$product->qty),
                    anchor($this->title.'/barcode/'.$product->id,'<span>barcode</span>', $this->atts).' &nbsp; '.    
	...
	$data['form_action'] = site_url($this->title.'/search');
        ...
        if ($this->input->post('cbarcode') == 0)
        {
            $val = explode('PRO-0', $this->input->post('tsku'));
            if (@$val[1]){ $sku = $val[1]; }else { $sku = null; }
        }
        else { $sku = $this->input->post('tsku'); }

	// ---------------------------------------- //
        $products = $this->Product_model->search($this->input->post('ctype'), $sku, $this->input->post('cstorage'), $this->input->post('cbrand'), $this->input->post('ccategory'), 
...
//               $this->load->view('template', $data);
    ...
    function calculate_qty()
    {
       ...
       {
          $open = $this->stock_ledger->get_trans($res->id, $p->month, $p->year, 'openqty'); 
          $trans = $wt->get_sum_transaction_qty($res->id,$p->month,$p->year);
//          $result = intval($open + $trans);
    ...
    function report_process()
    {
        ...
        $category = $this->input->post('ccategory');
        $brand    = $this->input->post('cbrand');
        $warehouse = $this->input->post('cwarehouse');
...
//        Property Details
    ...
    { 
        // $sum = $this->product->get_sum_stock($pid); metode FIFO
        $sum = $this->product->get_last_stock_price($pid);
