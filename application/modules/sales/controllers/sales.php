...
class Sales extends MX_Controller
{
	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last_sales');
            ...
            $this->table->set_heading('No', 'Type', 'Code', 'Cur', 'Date', 'Customer', 'Notes', 'Total', 'Balance', '#', 'Action');
            ...
            foreach ($saless as $sales)
            {
                ...
                $this->table->add_row
                (
                    ++$i, $sales->type, 'SO-00'.$sales->no, $sales->currency, tglin($sales->dates), $sales->prefix.' '.$sales->name, $this->cek_space($sales->notes), number_format($sales->total + $sales->costs), number_format($sales->p2), $this->status($sales->status),
    ...
    function confirmation($pid)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $this->contract->update_balance($sales->contract_no, intval($sales->total+$sales->discount), 0);
...
//                                         $sales->no, 'AR', $sales->discount, $sales->p2);
...
//        else { $this->session->set_flashdata('message', "This $this->title related to another component..!");  }
    ...
    private function rollback($uid,$po)
    {
      ...
      $sales = $this->Sales_model->get_sales_by_id($uid)->row();
      $this->contract->update_balance($sales->contract_no, $sales->total+$sales->discount, 1);  
	...
	// Form validation
        ...
        $this->form_validation->set_rules('ttaxno', 'Tax - No', 'required|numeric|callback_valid_tax_no['.$this->input->post('ctype').']');
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            $sales = array('customer' => $this->contract->get_contract_customer($this->input->post('tcontract')), 
                           'type' => $this->input->post('ctype'), 'contract' => 1, 'contract_no' => $this->input->post('tcontract'), 'invno' => $this->input->post('ttaxno'),
...
//            echo validation_errors();
	...
	$data['form_action'] = site_url($this->title.'/update_process/'.$pid);
        ...
        $data['code'] = $sales->no;
        $data['default']['taxno'] = $sales->invno;
        $data['default']['type'] = ucfirst($sales->type);
...
//    ======================  Item Transaction   ===============================================================
    ...
    private function total($size,$coloumn,$price,$discount,$tax,$count,$round)
    {
        if ($count == 0)
        {
            ...
            if($round == 'round') { $tax = round($this->tax->calculate_tax($res,$tax)); }else { $tax = floor($this->tax->calculate_tax($res,$tax)); }
            $amount = $netto + $tax;
...
//            $this->load->view('sales_transform', $data);
    ...
    public function valid_tax_no($no,$type)
    {
        $ps = new Period_lib();
        if ($this->Sales_model->valid_tax_no($no,$type,$ps->get('year')) == FALSE)
        {
            $this->form_validation->set_message('valid_tax_no', "Tax Order No already registered.!");
            return FALSE;
        }
        else {  return TRUE; }
    }
...
//        $data['table'] = $this->table->generate();
   ...
   function print_invoice($pid=null,$type=null)
   {
       ...
       $data['type'] = ucfirst($sales->type);
       $data['taxno'] = $sales->invno;
       ...
       $data['phone'] = $sales->phone1.' / '.$sales->phone2;
...
//       customer
   ...
   function tax_invoice($pid=null,$page=1)
   {
       ...
       $data['podate'] = tglincomplete($sales->dates);
       $data['fakdate'] = tglincomplete($sales->dates,'tax');
       $data['customer'] = strtoupper($sales->prefix.' '.$sales->name);
       $data['desc'] = $sales->desc;
       $data['notes'] = $sales->notes;
       $data['user'] = $this->user->get_username($sales->user);
...
// ====================================== REPORT =========================================
 ...
 {
     $ps = new Period_lib();
     echo $this->Sales_model->counter_tax($this->input->post('type'),$ps->get('year'));
 }
