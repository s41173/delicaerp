...
$(document).ready(function(){
	...
	$('#ctype').change(function() {
	   ...
	   if (type)
	   {
		    $.ajax({
			type: 'POST',
			url: site +"/sales/get_tax_no",
			data: "type="+ type,
			success: function(data)
			{
			   document.getElementById("ttaxno").value = data;
			}
			})
			return false; 
	   }
	   else { document.getElementById("ttaxno").value = '0'; }
...
function open_window()
{
	...
	if (type != "")
	{
		if (type == 'tax'){ var urlemployee = (site+'/contract/get_list/tax'); }
		else if (type == 'non') { var urlemployee = (site+'/contract/get_list/non'); }
    	window.open(urlemployee,'Contract','height=400,width=600,scrollbars=yes,resizable=yes');
    }
	else { alert("Select Sales Type First....!!"); }
	...
	<form name="modul_form" class="myform" id="form" method="post" action="<?php echo $form_action; ?>" >
				<table>
                    ...
                    <tr> 
						...
						<td> <select name="ctype" id="ctype"> <option value=""> -- </option> <option value="tax"> Tax </option> <option value="non"> Non </option> </select> &nbsp;
                             <input type="text" name="ttaxno" id="ttaxno" size="3" maxlength="5" onkeyup="checkdigit(this.value, 'ttaxno')" />
                        </td>
					</tr>
