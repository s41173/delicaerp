...
class Csales extends MX_Controller
{
    function __construct()
    {
        ...
        $this->sr         = $this->load->library('sales_return_lib');
        $this->stocktemp  = new Sales_stock_temp_lib();
    ...
    private $properti, $modul, $title, $currency, $stocktemp, $stockvalue=0;
	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last_sales');
            ...
            foreach ($saless as $sales)
            {
                ...
                $this->table->add_row
                (
                    ++$i, 'CSO-0'.$sales->no, $sales->currency, tglin($sales->dates), $sales->prefix.' '.$sales->name, $this->cek_space($sales->notes), number_format($sales->total + $sales->costs), number_format($sales->p2), $this->status($sales->status),
    ...
    function get_list($currency='IDR',$customer=null,$st=0)
    {
        ...
        foreach ($saless as $sales)
        {
           $data = array(
            ...
            $this->table->add_row
            (
                ++$i, 'CSO-0'.$sales->no, tglin($sales->dates), $sales->notes, number_format($sales->total), number_format($sales->p2),
                form_button($data)
            );
        }
...
//    ===================== approval ===========================================
    ...
    function confirmation($pid)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                //  create journal
               $this->create_po_journal($sales->id,$sales->dates, $sales->currency, 'CSO-0'.$sales->no.'-'.$sales->notes, 'CSJ',
                                         $sales->no, 'AR', $sales->total + $sales->costs, $sales->p1,$sales->p2);

               $this->session->set_flashdata('message', "$this->title CSO-0$sales->no confirmed..!"); // set flash data message dengan session
               redirect($this->title);
    ...
    private function stock($pid,$req,$so)
    {
          ...
          if ($res != null)
          {
             ...
             { 
                 ...
                 $this->stocktemp->min_stock($pid,$res->dates,$sales->dates,$res->qty,$so);
                 $this->get_stock($pid, intval($req - $res->qty),$so); 
             }
             else 
             { 
                 $this->stockvalue = $this->stockvalue + intval($req*$res->amount);
                 $this->stocktemp->min_stock($pid,$res->dates,$sales->dates,$req,$so);
    ...
    private function update_product($so)
    {
        ...
        foreach ($val as $res)
        {
            $this->product->min_qty($res->product,$res->qty, $this->product->get_amount_stock($so));

            $this->wt->add($sales->dates, 'CSO-0'.$sales->no,
    ...
    private function unupdate_product($so)
    {
        ...
        foreach ($val as $res)
        {
            $this->product->add_qty($res->product,$res->qty,$this->product->get_amount_stock($so));
            $this->stocktemp->rollback_stock($so);
            $this->wt->remove($sales->dates, 'CSO-0'.$sales->no, $res->product);
        }
    }
...
//    ===================== approval ===========================================
    ...
    private function create_po_journal($sid,$date,$currency,$code,$codetrans,$no,$type,$amount,$p1,$p2)
    {
        ...
        {  
           // create journal- GL
           $this->journalgl->new_journal('0'.$no,$date,'SJ',$currency,$code,$amount, $this->session->userdata('log'));
           $this->journalgl->new_journal('0000'.$no,$date,'CR',$currency,'Customer DP Payment : SJ-00'.$no,$p1, $this->session->userdata('log'));
           
           $jid = $this->journalgl->get_journal_id('SJ','0'.$no);
           $dpid = $this->journalgl->get_journal_id('CR','0000'.$no);
    ...
    function delete($uid,$po)
    {
        ...
        {
            if ($this->valid_period($sales->dates) == TRUE && $this->sr->cek_relation($po, 'sales') == TRUE ) // cek journal harian sudah di approve atau belum
            {
                if ($sales->approved == 1)
                {
                   $this->unupdate_product($po); 
                   $this->journalgl->remove_journal('SJ', '0'.$po); // journal gl
                   $this->journalgl->remove_journal('CR', '0000'.$po);
                   
                   $data = array('approved' => 0);
                   $this->Csales_model->update_id($uid, $data);
                   $this->session->set_flashdata('message', "1 $this->title successfully rollback..!");
                }
                else
                {
                   $this->sitem->delete_po($po); 
                   $this->Csales_model->delete($uid); 
...
//        $data['table'] = $this->table->generate();
   ...
   function print_invoice($po=null,$type=null)
   {
       ...
       //sales
       $data['pono'] = 'CSO-0'.$po.'/'.$this->get_romawi(date("m",strtotime($sales->dates))).'/'.date("Y",strtotime($sales->dates));
       ...
       $this->load->view('sales_order_invoice', $data);
