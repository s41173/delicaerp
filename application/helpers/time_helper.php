...
function tglincomplete($tgl,$type=null)
{
    if ($tgl != "")
    {
        // Konversi hari dan tanggal ke dalam format Indonesia
        if ($type == 'tax')
        {
           $tgl = date('d', strtotime($tgl)).' '.get_month_indo(date('m', strtotime($tgl))).' '.date('Y', strtotime($tgl));
        }
        else { $tgl = date('d F Y', strtotime($tgl)); }
